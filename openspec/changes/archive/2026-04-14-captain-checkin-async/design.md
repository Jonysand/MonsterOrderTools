...
| `TextToSpeech.cpp/h` | `asyncPendingQueue_` 改为多并发支持（`activeRequestCount_` 计数器，上限3）；修改 `Tick()` 检查逻辑；添加 `IsPlaying()` 检查防止打断播放；`playbackStarted` 标志；`PLAYBACK_TIMEOUT_SECONDS = 0` |
| `MiniMaxTTSProvider.cpp` | 删除 `cv.wait()` 阻塞，改为真正异步回调 |
| `XiaomiTTSProvider.cpp` | 删除 `cv.wait()` 阻塞，改为真正异步回调 |
| `AudioPlayer.cpp/h` | `IsPlaybackComplete()` 中重置 `playing = false`；`playing` 改为 `mutable`；播放时删除旧 temp 文件 |
